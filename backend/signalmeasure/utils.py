...
class SignalStrength(PacketManager):
    ...
    def wifi_strength(cls, interface, duration):
        try:
            ...
            print(f"Interface {interface} not found. Searching for available interfaces...")
            ...
            for alt_interface in available_interfaces:
                ...
                sniff(iface=alt_interface, prn=cls._packet_handler, timeout=duration, store=False)
                continue
            ...
            amplitudes = [m[0] for m in cls.measurements if isinstance(m[0], (int, float))]
            frequencies = [m[1] for m in cls.measurements if isinstance(m[1], (int, float))]
